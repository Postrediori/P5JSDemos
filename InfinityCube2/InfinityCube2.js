...
function draw() {
  ...
  drawOutline(textureBuffer, FOREGROUND, R);
}
...
function drawOutline(ctx, clr, r) {
  textureBuffer.fill(clr);
  textureBuffer.noStroke();    
  textureBuffer.beginShape(TESS);
  
  textureBuffer.vertex(W/2, 0);
  textureBuffer.vertex(W/2, -W/2);
  textureBuffer.vertex(-W/2, -W/2);
  textureBuffer.vertex(-W/2, W/2);
  textureBuffer.vertex(W/2, W/2);
  textureBuffer.vertex(W/2, 0);
  textureBuffer.vertex(r, 0);
  
  const N = 100;
  for (let i=0; i<=N; i++) {
    let a = radians(360/N * i);
    textureBuffer.vertex(r * cos(a), r * sin(a));
  }
  
  textureBuffer.endShape(CLOSE);
}
